...
RegisterServerEvent('dx-secondjob:getsecondjob')
AddEventHandler('dx-secondjob:getsecondjob', function()
    ...
    MySQL.Async.fetchAll('SELECT job2, job2_grade FROM users WHERE identifier = @identifier', { ['@identifier'] = xPlayer.getIdentifier() }, function(result)
        ...
        if result[1] ~= nil and result[1].job2 ~= nil and result[1].job2_grade ~= nil then
                TriggerClientEvent('dx-secondjob:returnsecondjob', _source, result[1].job2, result[1].job2_grade)
        else
            local msg = _U('error_loading')
            local type = 'error'
            TriggerClientEvent('dx-secondjob:notification',source,type,msg)
        end
    end)
end)

RegisterServerEvent('dx-secondjob:setsecondjob')
AddEventHandler('dx-secondjob:setsecondjob', function(job1, job1_grade, job2, job2_grade)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    if havePermission(xPlayer)then
        if not args[1] or not args[2] or not args[3] then
            ...
            if not xTarget then
                ...
                local type2 = 'error'
                TriggerClientEvent('dx-secondjob:notification',source,type2,msg2)
            else
                    ...
                    MySQL.Async.execute('UPDATE users SET job2 = @job2, job2_grade = @job2_grade WHERE identifier = @identifier',
                        { 
                            ...
                            function(affectedRows)
                                if affectedRows == 0 then
                                    local type3 = 'error'
                                    TriggerClientEvent('dx-secondjob:notification',source,type3,_U('error'))
                                    print('Player with steam ID: '..xPlayer.getIdentifier()..' had an issue while setting setjob to other player')
                                else
                                    local type5 = 'success'
                                    TriggerClientEvent('dx-secondjob:notification',source,type5,_U('setjob', GetPlayerName(tonumber(args[1]))))
                                end
                            end
                    )
                else
                    local type4 = 'error'
                    TriggerClientEvent('dx-secondjob:notification',source,type4,_U('job_error'))
                end
            end
        end
    else
        TriggerClientEvent('dx-secondjob:notification',source,"error",_U('no_permissions'))
    end
end, false)
